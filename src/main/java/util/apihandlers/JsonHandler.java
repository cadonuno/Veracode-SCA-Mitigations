...
public class JsonHandler {
    public static Optional<List<ApplicationProfile>> getApplicationProfilesFromPayload(JSONObject apiCallResult) {
    ...
    public static Optional<ApplicationProfile> getApplicationProfileFromPayload(JSONObject apiCallResult) {
        return Optional.of(apiCallResult)
    ...
    private static List<ApplicationProfile> getAllApplications(JSONArray allApplications) {
        List<ApplicationProfile> foundApplications = new ArrayList<>();
        for (int currentIndex = 0; currentIndex < allApplications.length(); currentIndex++) {
            tryGetElementAtJsonArrayIndex(allApplications, currentIndex)
                    .map(JsonHandler::getApplication)
                    .ifPresent(foundApplications::add);
        }
    ...
    public static ApplicationProfile getApplication(JSONObject applicationNode) {
        return new ApplicationProfile(
                ...
                tryGetElementAsString(applicationNode, "app_profile_url").orElse(""));
    }
